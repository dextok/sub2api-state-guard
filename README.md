...
| 票池 | 模型列表（一行一个）、每个池的票数、票有效期与同批错峰、补池阈值、检查间隔、探针超时与推理强度、每轮代理数、重试轮数、是否直连、网关地址、探针 UA。 |
...
| 票池长期 0/5，分级全是「未确认」 | 上游一个模型名都没回报，插件确认不了撞到的票属于哪个模型，所以一张都不收。先确认「网关地址」填的是不是真正的 codex 网关（中转层剥掉 `response.created` 事件也会这样）。 |
