...
- **票按 (账号 × 模型) 分池，绝不跨模型复用。** 满血票的基本标准是两条：**HTTP 200 有产出，
  且上游自报的模型就是请求的那个模型**。此外可以在高级设置里填一个**满血票长度**
  （默认 292）再多卡一道：`x-codex-turn-state` 长度对不上的判 `mismatch`，不入池。
  这一项**可以清空**（等于不按长度过滤）——满血长度因模型、因上游版本而异
  （实测 gpt-5.5 是 292，5.6 / 6 系列是 312，还会随上游变动），拿它当硬标准会误杀。
- **上游换模型服务的票不入池。** 账号在上游被降级时，上游会用别的模型（例如用
  gpt-5.6-luna 服务 gpt-6-astra）应答，铸出来的 state 看上去毫无异样。探针解析 SSE 里
  上游自报的模型名，对不上就判 `downgraded` 并丢弃——这类降级是账号在上游的权限问题，
  插件层修不了，但至少不会把别的模型的票当成满血票注给下一个请求。
  上游一个模型名都没回报时判 `unverified`，同样不入池：确认不了归属的票不如不要。
...
每张卡片的说明文字都收在标题右边那颗信息按钮里——鼠标扫到标题上按钮才浮现，悬浮（或用键盘聚焦）
它才展开说明，所以下面这张表也是那些说明的出处。

| 区块 | 说明 |
| --- | --- |
| 右上角 | 「启用过载防护」总开关（关闭后插件仍接管出站传输，但不再采集凭据、不撞票、不注入），旁边是「接管 N / M 个账号」。 |
| 实时票况 | 看板：总览药丸（票池水位 / 账号 / 代理库 / 注入头名）、每个账号的凭据状态、每个模型一张卡片（**点卡片**会列出这个池手里每一张有效票：指纹、长度、票龄、剩余有效期、撞到它的出口，并标出下一次注入要用的那张；弹窗跟着自动刷新一起带新）。每条代理的撞票成败在下面代理表的「状态」列里看，看板不再重复列一遍。**打开本页时先走一次 `config.test`**（宿主可能要求二次验证）把卡片组填出来，之后在 sub2api ≥ 0.2.7 上**每 10 秒**经只读通道自动刷新，右上角「刷新状态」可以立刻读一次（见 §4.1）。 |
| 票池 | 模型清单（芯片 + 「+ 添加模型」，**增删即时生效**，不用点保存；删除点芯片上的 ×）、「自动跟踪请求里出现的新模型」开关，以及默认折叠的**高级设置**：每个池的票数、票有效期与同批错峰、补池阈值、检查间隔、探针超时与推理强度、每轮代理数、重试轮数、满血票长度（默认 292，**留空则不按长度过滤**；其余数字框留空是恢复默认值）。`include_direct`（每轮也用直连打一发）、`gateway_base_url`、`user_agent` 用默认值即可，页面上不展示，要改走管理端的插件配置接口。 |
...
| 底部 | 「保存」写入并热切换配置（已经撞到的票和已采到的凭据不会丢）；「放弃修改」重新载入已保存的配置。模型的增删不经过这两个按钮——它们各自单独提交一次配置，点完当场生效，页面上别的未保存改动既不会被顺带提交，也不会被冲掉。 |
...
看板展示的始终是**已保存并生效**的那份配置的状态。刚用「+ 添加模型」加进去的模型即时生效，
但运行时数据要等下一次刷新才跟上，所以它会先以一张虚线卡片占位。
...
- 校验是**硬失败**而不是静默夹取：填超范围的值会直接保存失败并给出提示。主要范围——
  每模型票数 1–20，票有效期 300–86400 秒，补池冷却 30 秒 ~ 票有效期，检查间隔 10–3600 秒，
  探针超时 5–300 秒，每轮代理 0–32，重试 0–10 轮；账号 ≤ 500 个，池模型 ≤ 16 个，
  代理 ≤ 200 条（备注 ≤ 40 字节，地址不可重复）。
- `include_direct`（每轮也用直连打一发）关掉、而代理一条都没启用（或「每轮代理数」是 0）时**保存会被拒绝**：那样撞票永远发不出去。它默认是开的，页面上不展示。
...
| 分级 | 判定 | 入池 |
| --- | --- | --- |
| `healthy` 满血 | 200 + 有产出 + 上游自报的模型与请求一致 | ✅ 唯一入池的一档 |
| `downgraded` 降级 | 200 + 有产出，但上游自报的模型不是请求的那个（账号在上游被降级） | ❌ |
| `mismatch` 长度不符 | 模型对得上，但 state 长度与配置的「满血票长度」不符（该项留空时不会出现这一档） | ❌ |
| `unverified` 未确认 | 200 + 有产出，但上游一个模型名都没回报，确认不了这张票属于谁 | ❌ |
...
上游自报的模型名从 `response.created`（它一定早于首个 delta）或 `response.completed` 里取；
上游没给这个字段时判 `unverified`，不回落到长度。这个名字是外部输入，认不出是合法模型名就不回显原文。
...
关掉代理池的同时又把 `include_direct` 设成 false 则会在保存时被拒绝。
...
1. 用管理端的插件配置接口把 `gateway_base_url` 指向假桩 `http://127.0.0.1:9701/backend-api/codex`；配置页的代理表格里
...
2. 在插件管理页按 100% 灰度启用，用该 OAuth 账号发一次 Codex 请求。
   ...
   > 撞票端点取自**过路请求的 URL**，优先于配置里的 `gateway_base_url`。所以要让撞票也打到假桩，
...
5. 摆出异常形态重启假桩，看看看板怎么显示：
   ...
   ```bash
   go run ./tools/mockgateway -downgrade gpt-6-astra=gpt-5.6-luna  # 上游换模型服务（降级，不入池）
   ...
   `-state-length 292,312` 与 `-alt-length-models` 可以让长度变化：满血票长度留空时
   两种长度都照收，填上其中一个就只收那一种，另一种判 `mismatch`。
...
| 票池长期 0/5，分级全是「未确认」 | 上游一个模型名都没回报，插件确认不了撞到的票属于哪个模型，所以一张都不收。先确认撞票打到的是不是真正的 codex 网关（端点取自过路请求的 URL；中转层剥掉 `response.created` 事件也会这样）。 |
| 票池长期 0/5，分级全是「降级」 | 上游正在用别的模型服务这个模型的请求（卡片 detail 里写了是谁替谁）。这是账号在上游的模型权限问题，**插件层改不了**：换代理也没用。先确认账号是否被限流或降级；在恢复之前这个池会一直是空的，请求按 `on_model_unmatched` 策略处理。 |
| 票池长期 0/5，分级全是「长度不符」 | 撞到的票模型对得上，但长度与设定的「满血票长度」不符。满血长度因模型、因上游版本而异，点开卡片看一眼实际长度，把高级设置里的那一项改成它，或者直接清空（清空 = 不按长度过滤）。 |
| 分级全是「被拦」或「弱」 | 账号被限流或凭据已失效（令牌过期）。看板上的「最近一轮」摘要里有各出口的状态码。让账号再跑一次真实请求以刷新凭据。 |
| 分级全是「出错」 | 连响应都没拿到：代理全挂了，或网关不通。检查代理库条数与「最近一轮」摘要（出口地址按段打码），再看代理表格里哪几条只涨失败数。 |
| 某条代理只涨失败数 | 插件不会替你剔除它（这是刻意的，见 §5.3）。确认地址、协议、账号密码没写错；确实废了就在表格里删掉或停用。 |
| 代理库一直 0 条 | 表格里一条都没填，或全都停用了，或代理池总开关关着。看板总览药丸里的「代理库」就是启用的条数。 |
| 保存报「已开启过载防护的账号无法撞票」 | 配置里的 `include_direct` 是关的，代理又一条都没启用——撞票没有任何出口。启用一条代理，或走管理端的插件配置接口把 `include_direct` 打开。 |
| 保存报「不支持的代理协议」 | 只认 `socks5` / `socks5h` / `http` / `https`；`socks4` 等一律拒绝。报错里**不会**回显你填的地址（它可能带密码）。 |
| 请求返回 `OVERLOAD_GUARD_MODEL_UNMATCHED` | 有别的模型的票，但没有这次请求的模型（或请求体里读不出模型名），且策略是 `fail_request`（`request_sent=false`）。在配置页用「+ 添加模型」把它加进清单（即时生效），或开着「自动跟踪请求里出现的新模型」等它自己建池，或把策略改回 `passthrough`。 |
