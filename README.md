...
| 实时票况 | 看板：总览药丸（票池水位 / 账号 / 代理库 / 注入头名）、每个账号的凭据状态、每个模型一张卡片。每条代理的撞票成败在下面代理表的「状态」列里看，看板不再重复列一遍。**打开本页时先走一次 `config.test`**（宿主可能要求二次验证）把卡片组填出来，之后在 sub2api ≥ 0.2.7 上**每 10 秒**经只读通道自动刷新，右上角「刷新状态」可以立刻读一次（见 §4.1）。 |
| 票池 | 模型清单（芯片 + 「+ 添加模型」，**增删即时生效**，不用点保存；上方票况卡片点一下也能删）、「自动跟踪请求里出现的新模型」开关，以及默认折叠的**高级设置**：每个池的票数、票有效期与同批错峰、补池阈值、检查间隔、探针超时与推理强度、每轮代理数、重试轮数。`include_direct`（每轮也用直连打一发）、`gateway_base_url`、`user_agent` 用默认值即可，页面上不展示，要改走管理端的插件配置接口。 |
