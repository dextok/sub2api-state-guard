...
- **票按 (账号 × 模型) 分池，绝不跨模型复用。** 只有长度恰好等于该模型「满血票长度」的
  `x-codex-turn-state` 才入池。**满血长度因模型而异**：实测 gpt-5.5 是 292，
  gpt-5.6-sol / terra / luna 与 gpt-6-astra 是 312，所以长度按模型配置
  （`model_state_lengths`），没单列的模型用一个全局兜底值（默认 292）。
- **上游换模型服务的票不入池。** 账号在上游被降级时，上游会用别的模型（例如用
  gpt-5.6-luna 服务 gpt-6-astra）应答，铸出来的 state 长度照样对得上。探针解析 SSE 里
...
| 实时票况 | 看板：总览药丸（票池水位 / 账号 / 代理库 / 注入头名 / 默认满血长度）、每个账号的凭据状态、每个模型一张卡片，以及每条代理的撞票成败。**打开本页时先走一次 `config.test`**（宿主可能要求二次验证）把卡片组填出来，之后在 sub2api ≥ 0.2.7 上**每 10 秒**经只读通道自动刷新，右上角「刷新状态」可以立刻读一次（见 §4.1）。 |
| 票池 | 模型列表（一行一个）、按模型的满血长度（一行一条 `模型名=长度`）、每个池的票数、默认满血票长度、票有效期与同批错峰、补池阈值、检查间隔、探针超时与推理强度、每轮代理数、重试轮数、是否直连、网关地址、探针 UA。 |
...
- 校验是**硬失败**而不是静默夹取：填超范围的值会直接保存失败并给出提示。主要范围——
  每模型票数 1–20，满血长度 0（不判定）或 1–4096（默认值与每条按模型覆盖值同此范围），
  票有效期 300–86400 秒，补池冷却 30 秒 ~ 票有效期，检查间隔 10–3600 秒，探针超时 5–300 秒，
  每轮代理 0–32，重试 0–10 轮；账号 ≤ 500 个，池模型 ≤ 16 个，按模型的长度覆盖 ≤ 32 条
  （比池模型宽，因为「自动跟踪」出来的模型也可以单独配长度），
...
| 分级 | 判定 | 入池 |
| --- | --- | --- |
| `healthy` 满血 | 200 + 有产出 + 上游自报的模型与请求一致 + 长度等于该模型的满血长度 | ✅ 唯一入池的一档 |
| `downgraded` 降级 | 200 + 有产出，但上游自报的模型不是请求的那个（账号在上游被降级） | ❌ |
| `mismatch` 长度不符 | 200 + 有产出，但长度不是该模型的满血长度 | ❌ |
...
票不会被消费，有效期内可重复注入。改了某个模型的「满血票长度」之后，旧口径下收进来的票会在下一次读池时被清掉。
...
5. 摆出异常形态重启假桩，看看看板怎么显示：
   ...
   ```bash
   go run ./tools/mockgateway -mismatch-models gpt-6-astra      # 长度不符（不入池）
   ...
   注意假桩的 state 长度要和配置里该模型的「满血票长度」对得上，否则票全被判成长度不符：
   页面上「按模型的满血长度」里没列到的模型，用的是下面那个默认值。
...
| 票池长期 0/5，分级全是「长度不符」 | 上游给这个模型铸的票长度和配置里的口径对不上。满血长度因模型而异（实测 gpt-5.5=292，gpt-5.6-sol / terra / luna 与 gpt-6-astra=312），在「按模型的满血长度」里给它单独配一条即可；看板卡片上会显示这个池当前用的口径。 |
| 票池长期 0/5，分级全是「降级」 | 上游正在用别的模型服务这个模型的请求（卡片 detail 里写了是谁替谁）。这是账号在上游的模型权限问题，**插件层改不了**：改长度、换代理都没用。先确认账号是否被限流或降级；在恢复之前这个池会一直是空的，请求按 `on_model_unmatched` 策略处理。 |
