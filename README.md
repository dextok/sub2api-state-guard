...
- **票按 (账号 × 模型) 分池，绝不跨模型复用。** 满血票的标准只有两条：**HTTP 200 有产出，
  且上游自报的模型就是请求的那个模型**。`x-codex-turn-state` 的长度不参与判定——
  它因模型、因上游版本而异（实测 gpt-5.5 是 292，5.6 / 6 系列是 312，还会随上游变动），
  拿它当标准只会误杀或误收。
...
| 票池 | 模型清单（芯片 + 「+ 添加模型」，**增删即时生效**，不用点保存；删除点芯片上的 ×）、「自动跟踪请求里出现的新模型」开关，以及默认折叠的**高级设置**：每个池的票数、票有效期与同批错峰、补池阈值、检查间隔、探针超时与推理强度、每轮代理数、重试轮数。`include_direct`（每轮也用直连打一发）、`gateway_base_url`、`user_agent` 用默认值即可，页面上不展示，要改走管理端的插件配置接口。 |
...
上游没给这个字段时不判降级，回落到长度判定。这个名字是外部输入，认不出是合法模型名就不回显原文。
...
5. 摆出异常形态重启假桩，看看看板怎么显示：
   ...
   `-state-length 292,312` 与 `-alt-length-models` 可以让长度变化——票照样入池，
   长度不参与满血判定。
